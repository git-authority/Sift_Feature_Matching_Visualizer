...
DRAW_MAX = 150            # limit number of drawn matches for readability
MAX_WIDTH = 900           # downscale images to this width
SIFT_NFEATURES = 2000
BOTTOM_MARGIN = 70        # extra pixels below image where centered label will be printed
...
def draw_matches_single(img1, kp1, img2, kp2, matches, inlier_mask=None, draw_max=DRAW_MAX):
    ...
    random.seed(1)
    palette = [(int(255*random.random()), int(255*random.random()), int(255*random.random())) for _ in range(len(indices))]
    ...
    for idx_idx, i in enumerate(indices):
        ...
        if inlier_mask is not None:
            if inlier_mask[i]:
                color = (0,200,0)  # green for inliers
                thickness = 2
            else:
                color = (200,0,0)  # red for outliers
                thickness = 1
        x1,y1 = map(int, kp1[m.queryIdx].pt)
        x2,y2 = map(int, kp2[m.trainIdx].pt)
        x2s = x2 + w1
        cv2.circle(canvas, (x1,y1), 3, color, -1)
        cv2.circle(canvas, (x2s,y2), 3, color, -1)
        cv2.line(canvas, (x1,y1), (x2s,y2), color, thickness)
...
def process_dataset(folder):
    ...
    if len(imgs) < 2:
        ...
        cv2.putText(blank, text, (20,50), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0,0,0), 2, cv2.LINE_AA)
    ...
    # choose drawing mask: prefer inliers if enough
    ...
    canvas = draw_matches_single(img1, kp1, img2, kp2, good, inlier_mask=used_mask, draw_max=draw_count)

    # (TOP-LEFT HEADER REMOVED) -> do NOT draw any header inside the image

    # create new image with bottom margin and draw same label centered in that bottom margin
    ch, cw = canvas.shape[:2]
    new_h = ch + BOTTOM_MARGIN
    new_canvas = np.ones((new_h, cw, 3), dtype=np.uint8) * 255
    new_canvas[0:ch, 0:cw] = canvas

    # prepare centered text in bottom margin (three pieces joined by " | ")
    bottom_text = " | ".join([
    ...
    cv2.putText(new_canvas, bottom_text, (text_x+1, text_y+1), font, font_scale, (255,255,255), 3, cv2.LINE_AA)
    cv2.putText(new_canvas, bottom_text, (text_x, text_y), font, font_scale, (0,0,0), thickness, cv2.LINE_AA)

    cv2.imwrite(out_file, new_canvas)
    print(f"[OK] Saved: {out_file}  (kp: {kp1_count} vs {kp2_count}, good: {len(good)}, ransac_inliers: {ransac_inliers})")

def main():
    if not os.path.exists(DATASET_DIR):
        print(f"Dataset folder '{DATASET_DIR}' not found. Place Dataset/ next to this script.")
        return
    # detect dataset subfolders (process whatever is present)
    subfolders = sorted([os.path.join(DATASET_DIR, d) for d in os.listdir(DATASET_DIR)
